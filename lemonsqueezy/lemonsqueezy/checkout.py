...
def get_legacy_checkout_redirect_url(params):
    ...
    Only legacy Payment Request links are accepted; all client-supplied
    checkout parameters are ignored once the Payment Request is identified.
    """
    ...
    if reference_doctype and reference_doctype != "Payment Request":
        return None

    if reference_docname and payment_request_name != reference_docname:
        return None

    # Legacy links generated by Payment Request include order_id=self.name.
    # Require the same invariant before upgrading the link to a signed token.
    if order_id and order_id != payment_request_name:
        return None
